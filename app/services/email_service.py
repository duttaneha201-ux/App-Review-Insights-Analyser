    ...
    def _create_html_template(
        ...
        if date_range and len(date_range) == 2:
            ...
            if isinstance(start_date, str):
                ...
                try:
                    start_date = dt.fromisoformat(start_date).date() if 'T' in start_date else dt.strptime(start_date, '%Y-%m-%d').date()
                    end_date = dt.fromisoformat(end_date).date() if 'T' in end_date else dt.strptime(end_date, '%Y-%m-%d').date()
                except:
                    pass
            if hasattr(start_date, 'strftime'):
                date_range_str = f"<p style='color: #666; font-size: 14px; margin: 0 0 15px 0;'><strong>Review Period:</strong> {start_date.strftime('%B %d, %Y')} - {end_date.strftime('%B %d, %Y')}</p>"
        ...
        <tr>
            <td style="padding: 30px 20px; background-color: #ffffff;">
                <div style="max-width: 600px; margin: 0 auto;">
                    ...
                    {f'<div style="margin-bottom: 20px;"><p style="color: {styles["accent_color"]}; font-size: 18px; font-weight: bold; margin: 0 0 10px 0;">📱 {self._escape_html(app_name)}</p></div>' if app_name else ''}
                    ...
                    <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; text-align: center;">
                        ...
                        <p style="color: #999; font-size: 11px; margin: 15px 0 5px 0;">
                            <a href="{unsubscribe_url or 'https://app-review-insights-analyser.streamlit.app/unsubscribe'}" style="color: #999; text-decoration: underline;">Unsubscribe from these emails</a>
                        </p>
                    </div>
                </div>
            </td>
        ...
        if date_range and len(date_range) == 2:
            ...
            if isinstance(start_date, str):
                ...
                try:
                    ...
                    pass
            if hasattr(start_date, 'strftime'):
                date_range_text = f"\nReview Period: {start_date.strftime('%B %d, %Y')} - {end_date.strftime('%B %d, %Y')}\n"
        
        text = f"""
WEEKLY PRODUCT PULSE{' - ' + app_name if app_name else ''}
{datetime.now().strftime('%B %d, %Y')}
{date_range_text}{app_header}
{pulse.title}
...
"""
        ...
        unsubscribe_link = unsubscribe_url or "https://app-review-insights-analyser.streamlit.app/unsubscribe"
        text += f"""
---
Generated by App Review Insights Analyzer
Word count: {pulse.word_count()} / 250
...
"""
    ...
    def send_weekly_pulse(
        ...
        unsubscribe_url = "https://app-review-insights-analyser.streamlit.app/unsubscribe"
        
        # Create email content
        html_content = self._create_html_template(pulse, app_name, audience, unsubscribe_url, date_range)
        text_content = self._create_text_template(pulse, app_name, unsubscribe_url, date_range)
